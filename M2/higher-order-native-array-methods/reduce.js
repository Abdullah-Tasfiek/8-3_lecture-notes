...
function stringConcat(arr) {
  let string = arr.reduce((accumulate, firstValue) => {
    if (typeof accumulate === 'number') {
      firstValue = firstValue.toString();
    }
    return accumulate + firstValue;
  }); 
  return string;
}
...
function shoppingSpree(arr) {
  let count = arr.reduce((accumulate, firstValue) => {
    return accumulate + firstValue.price;
  }, 0);
  return count;
}
const wishlist = [
...
function flatten(arr) {
  let count = arr.reduce((accumulate, firstValue) => {
    return accumulate.concat(firstValue)
  }, []);
  return count
}
...
//   { name: 'Zack', age: 19, voted: false },
// ];
//how many of the age, and how many of them voted
